...
export default function makeNodeTestSuite(projectBaseDir, testPath, //rel to projectBaseDir
opts) {
...
class NodeTestSuite extends BaseTypes.TestSuite {
    ...
    //array elements
    appendTestInfos(testOut, infos, isTestCase = false, labels = '') {
...
function makeNodeTestCase(projectBaseDir, testPath, opts) {
...
if (DO_CLI_TEST) {
    async function cliTest() {
        ...
        if (!testPath) {
            E.panic(`usage: ${process.argv[1]} JUNIT_XML_PATH`);
        }
        const suite = new NodeTestSuite('.', testPath, { name: 'CLI Tests', isCliTest: true });
        const result = await suite.run();
        if (!result.isOk)
            E.panic(result.err.toString());
        console.dir(result.val, { depth: null });
    }
    await cliTest();
}
//# sourceMappingURL=node-test-suite.js.map